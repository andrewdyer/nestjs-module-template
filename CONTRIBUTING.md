...
4. Run tests with `npm test` to verify that the environment is working correctly. You can also use `npm run test:watch` to run the tests in watch mode, and `npm run test:coverage` to generate a coverage report.
...
- Run `npm run lint` to check for linting errors and ensure code quality.
- Run `npm run type-check` to ensure there are no TypeScript type errors.
- Organize imports into clearly separated groups, following this order: third-party packages, app-level aliases, and local imports.
...
- Build the project with `npm run build`, which generates the output files in the `dist/` folder.
...
2. In the target project's directory, add this package with `yalc add @your-scope/your-module`.
...
5. Alternatively, manually update the package in the target project with `yalc update @your-scope/your-module`.
